...
class AiInterceptorSmokeTest extends Command
{
    ...
    public function handle(AiAssistantInterceptor $interceptor): int
    {
        ...
        $surfacedIds = $out['clinics']->pluck('id')->all();
        // Verify the AI path bumped the LIVE impressions table (clinic_impressions,
        // source='ai') — not the legacy clinic_stats.search_appearances column.
        $aiImpressions = (int) DB::table('clinic_impressions')
            ->whereIn('clinic_id', $surfacedIds ?: [0])
            ->where('date', today())->where('source', 'ai')->sum('count');
        $this->line("  kind: {$out['kind']}");
        $this->line("  log rows before: {$beforeLogs}, after: {$afterLogs}");
        $this->line('  surfaced clinic ids: '.implode(',', $surfacedIds));
        $this->line("  clinic_impressions(source=ai, today) for surfaced: {$aiImpressions}");
