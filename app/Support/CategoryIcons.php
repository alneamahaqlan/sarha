...
/**
 * Canonical built-in icon library for specializations (categories).
 *
 * A category's `icon` column stores EITHER one of these keys (built-in vector
 * icon) OR an uploaded image path (e.g. "category-icons/abc.png"). The legacy
 * `emoji` column is mapped onto a key as a last-resort fallback so existing
 * categories keep rendering without an explicit icon.
 *
 * Keep this list in sync with the React mirror at
 * resources/react-admin/src/features/categories/category-icons.ts
 * (same keys + same SVG path data) so the admin picker previews match the
 * public rendering exactly.
 *
 * All paths are stroke-only (fill="none"), drawn on a 0 0 24 24 viewBox.
 */
final class CategoryIcons
{
    /** Built-in icons that ship with the platform, in picker display order. */
    public const PATHS = [
        // — Core medical specialties (the original set) —
        ...
        // — 20 extra icons the admin can pick from —
    ...
    /** Legacy emoji → key fallback for categories that have no explicit icon. */
    ...
    {
        return $key !== null && array_key_exists($key, self::PATHS);
    }

    /** The ordered list of built-in keys (for the admin picker). */
    public static function keys(): array
    {
        return array_keys(self::PATHS);
    }

    /** Resolve a stored emoji to a built-in key, or 'med' as the default. */
    public static function keyForEmoji(?string $emoji): string
    {
        return self::EMOJI_MAP[trim((string) $emoji)] ?? 'med';
    }
}
