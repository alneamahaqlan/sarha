...
@section('title', __('site.followed_clinics_page_title'))

@section('content')
@php $hasFilters = $q !== '' || $cityId || $catSlug; @endphp
<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">@lang('site.followed_clinics_page_title')</h1>
        <p class="text-sm text-gray-500 mt-1">@lang('site.followed_clinics_page_subtitle')</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <form method="GET" action="{{ route('account.following') }}"
                ...
                <div class="sm:col-span-2 lg:col-span-2">
                    <div class="relative">
                        ...
                        <input type="search" name="q" value="{{ $q }}"
                               placeholder="@lang('site.followed_clinics_search_ph')"
                ...
                @if($cities->isNotEmpty())
                    <select name="city" class="py-2.5 px-3 rounded-lg border border-gray-200 focus:border-sage-500 focus:ring-1 focus:ring-sage-500 text-sm bg-white">
                        <option value="">@lang('site.followed_filter_city')</option>
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" @selected((string) $cityId === (string) $city->id)>{{ $city->display_name ?? $city->name }}</option>
                        @endforeach
                    </select>
                @endif
                ...
                <div class="sm:col-span-2 lg:col-span-4 flex items-center gap-2">
                    ...
                    @if($hasFilters)
                        <a href="{{ route('account.following') }}" class="text-sm font-semibold text-gray-500 hover:text-gray-700">
            ...
            @if($clinics->total() === 0)
                <div class="bg-white rounded-xl shadow-sm p-10 text-center">
                    <x-icon name="building" class="w-16 h-16 mx-auto mb-3 text-gray-300" />
                    <p class="text-gray-500 mb-4">
                        {{ $hasFilters ? __('site.followed_clinics_empty_filtered') : __('site.followed_clinics_empty') }}
                    </p>
