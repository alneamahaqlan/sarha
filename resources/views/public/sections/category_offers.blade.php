{{-- Offers filtered by a single category. $data['category'] + $data['services']. --}}
@php
                        ...
                        <div class="p-5 flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <h3 class="font-semibold text-gray-800 line-clamp-2 group-hover:text-sage-700 transition-colors">{{ $service->name }}</h3>
                                    @if($discount > 0)
                                ...
                                <div class="mt-2 flex items-baseline gap-2">
                                    <span class="text-sage-700 font-bold">{{ number_format($service->price) }}<span class="text-xs font-normal ms-1">@lang('site.currency_sar')</span></span>
                                    @if($service->old_price)
                                        <span class="text-xs text-gray-400 line-through">{{ number_format($service->old_price) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
