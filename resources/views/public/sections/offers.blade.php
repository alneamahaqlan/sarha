{{-- Latest discounted services across all clinics. $data['services'] is a paginated collection.
     Each card: image (fallback to category emoji), discount %, price, clinic, CTA. --}}
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                @foreach($services as $i => $service)
                    ...
                    <div class="reveal" style="--reveal-delay:{{ ($i % 4) * 90 }}ms">
                        <a href="{{ $clinic ? route('clinic.show', $clinic->slug) : '#' }}"
                           class="block group bg-white rounded-2xl ring-1 ring-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all overflow-hidden h-full">
                            ...
                            <div class="p-4">
                                <h3 class="font-semibold text-gray-800 line-clamp-1 group-hover:text-sage-700 transition-colors">{{ $service->name }}</h3>
                                @if($clinic)
                                ...
                                <div class="mt-3 flex items-end justify-between gap-2">
                                    <div>
                                        @if($service->old_price)
                                            <span class="block text-[11px] text-gray-400 line-through">{{ number_format($service->old_price) }}</span>
                                        @endif
                                        <span class="text-sage-700 font-bold text-lg">{{ number_format($service->price) }}<span class="text-xs font-normal ms-1">@lang('site.currency_sar')</span></span>
                                    </div>
