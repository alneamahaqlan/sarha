...
<div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
    ...
    @if($whatsappUrl)
        <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener"
           ...
           class="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-2.5 px-3 rounded-lg font-semibold text-sm transition-colors">
    ...
    @if($directionsUrl)
        <a href="{{ $directionsUrl }}" target="_blank" rel="noopener"
           ...
           class="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2.5 px-3 rounded-lg font-semibold text-sm transition-colors {{ $clinic->phone && $whatsappUrl ? 'col-span-2 sm:col-span-1' : '' }}">
