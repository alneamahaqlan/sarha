{{--
    ...
    Sits together prominently so visitors can reach the complex in one tap.
...
@if($clinic->phone || $directionsUrl)
<div class="grid grid-cols-3 gap-2">
    {{-- Call --}}
    @if($clinic->phone)
        <a href="tel:{{ $clinic->phone }}"
           data-track="call" data-clinic="{{ $clinic->id }}"
           class="flex items-center justify-center min-h-touch gap-1.5 bg-sage-600 hover:bg-sage-700 text-white py-2.5 px-2 sm:px-3 rounded-lg font-semibold text-xs sm:text-sm transition-colors">
    ...
    @if($whatsappUrl)
        <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener"
           ...
           class="flex items-center justify-center min-h-touch gap-1.5 bg-green-500 hover:bg-green-600 text-white py-2.5 px-2 sm:px-3 rounded-lg font-semibold text-xs sm:text-sm transition-colors">
            ...
            <span>@lang('site.action_whatsapp')</span>
        </a>
    @endif
    ...
    @if($directionsUrl)
        <a href="{{ $directionsUrl }}" target="_blank" rel="noopener"
           ...
           class="flex items-center justify-center min-h-touch gap-1.5 bg-blue-600 hover:bg-blue-700 text-white py-2.5 px-2 sm:px-3 rounded-lg font-semibold text-xs sm:text-sm transition-colors">
            <x-icon name="map" class="w-4 h-4" />
            <span>@lang('site.action_directions')</span>
        </a>
    @endif
</div>
@endif
