...
<div class="flex items-center gap-2 flex-wrap" x-data="{ copied: false }">
    ...
    <a href="https://wa.me/?text={{ urlencode($clinic->name . ' — ' . $shareUrl) }}"
       ...
       class="w-11 h-11 rounded-full bg-green-50 text-green-600 hover:bg-green-100 flex items-center justify-center transition-colors"
       title="WhatsApp">
    ...
    <a href="https://twitter.com/intent/tweet?text={{ urlencode($clinic->name) }}&url={{ urlencode($shareUrl) }}"
       target="_blank" rel="noopener"
       class="w-11 h-11 rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 flex items-center justify-center transition-colors"
       title="X (Twitter)">
        𝕏
    </a>
    ...
    <button type="button"
            ...
            class="w-11 h-11 rounded-full bg-sage-50 text-sage-600 hover:bg-sage-100 flex items-center justify-center transition-colors"
