// Display-only: render a category's stored emoji as a professional vector icon.
// Mirrors resources/views/components/category-icon.blade.php (same paths/keys),
// so the public site and the admin panel show identical category icons.

const KEY_BY_EMOJI: Record<string, string> = {
  '🦷': 'dental',
  '✨': 'derma',
  '👁': 'eye',
  '👁️': 'eye',
...
export function CategoryIcon({ emoji, className = 'w-5 h-5' }: { emoji?: string | null; className?: string }) {
  const key = (emoji && KEY_BY_EMOJI[emoji.trim()]) || 'med';
  return (
    <svg
      className={className}
      fill="none"
